...
class CopilotAgentManager {
  ...
  async createAgent(name, model = 'gpt-4o-mini', promptContent = '', maxIterations = 0) {
    ...
    // NOTE: PRD.json will be generated by Ralph as the first task
    console.log(`[CopilotAgentManager]   ℹ prd.json will be generated by agent`);
...
1. FIRST: Read PROMPT.md and generate plans/prd.json (if it doesn't exist)
...
6. Output <promise>COMPLETE</promise> when all features are done

## Next Steps
- Read and analyze PROMPT.md
- Generate plans/prd.json from the prompt
- Begin implementation once PRD is created
...
`;
  ...
  /**
   * Generate a structured PRD from the user's prompt
   */
  generatePRDFromPrompt(name, promptContent, agentId) {
    ...
    return {
      meta: {
        version: '1.0.0',
        project: name,
        description: 'Agent task generated from user prompt',
        created: timestamp,
      ...
      objectives: {
        primary: `Complete the task described in PROMPT.md`,
        keyPrinciples: [
          'Work incrementally',
      ...
      features: [
        {
          ...
          status: 'not-started',
          requirements: [
            'Analyze PROMPT.md requirements',
  ...
  async startAgent(id) {
    ...
    // Build PowerShell command to run ralph loop via Copilot CLI
    const args = [
      '-NoProfile',
...
If all features are complete, output <promise>COMPLETE</promise>.`;
    ...
    // Write the loop prompt to a temp file
    const loopPromptPath = path.join(agent.workspaceDir, '.loop-prompt.md');
    await fs.writeFile(loopPromptPath, loopPrompt);
    
    args.push('-PromptFile', loopPromptPath);

    if (agent.maxIterations > 0) {
      args.push('-Iterations', agent.maxIterations.toString());
      console.log(`[CopilotAgent] Max iterations: ${agent.maxIterations}`);
    } else {
      // No iteration limit - run until completion marker detected
      console.log(`[CopilotAgent] Max iterations: unlimited (until <promise>COMPLETE</promise>)`);
    }
    
    args.push('-NotifyOnComplete');
    ...
    this.addLog(id, 'system', `Max Iterations: ${agent.maxIterations || 20}`);
    this.addLog(id, 'system', ``);
    this.addLog(id, 'system', `Loop Strategy:`);
    this.addLog(id, 'system', `  1. Read prd.json and progress.txt`);
    this.addLog(id, 'system', `  2. Pick highest-priority feature`);
    this.addLog(id, 'system', `  3. Implement feature incrementally`);
    this.addLog(id, 'system', `  4. Update PRD and progress`);
        ...
        lines.forEach(line => {
          if (line.trim()) {
            ...
            if (output.includes('<promise>COMPLETE</promise>')) {
              ...
              this.addLog(id, 'system', `PRD complete after ${agent.currentIteration} iteration(s)`);
              this.broadcast('copilot-agent:completed', { id, iterations: agent.currentIteration });
            }
          }
        });
      });
