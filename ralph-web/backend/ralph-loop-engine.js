...
/**
 * Ralph Loop Engine
 * Implements the autonomous agent loop using Ollama's native tool calling API
 */
class RalphLoopEngine {
  constructor(config, eventEmitter) {
    ...
    console.log(`[RalphLoopEngine] Initialized for workspace: ${this.workspaceDir}`);
    console.log(`[RalphLoopEngine] Model: ${this.model}`);
    console.log(`[RalphLoopEngine] Max iterations: ${this.maxIterations || 'unlimited'}`);
  }
  ...
  emitLog(message, level = 'info') {
    ...
    console.log(`[RalphLoopEngine] ${logEntry}`);
  ...
  async executeSingleTool(toolCall) {
    ...
    console.log(`[RalphLoopEngine] Tool args:`, args);
  ...
  async start() {
    ...
    try {
      ...
      this.emitLog('Ralph loop started', 'info');
      ...
      while (!this.shouldStop) {
        ...
        try {
          ...
          const response = await ollamaService.chatWithTools(
            this.model,
            this.conversationHistory,
            tools,
      ...
      this.emitLog('Ralph loop completed', 'success');
      return {
        success: true,
...
module.exports = RalphLoopEngine;
